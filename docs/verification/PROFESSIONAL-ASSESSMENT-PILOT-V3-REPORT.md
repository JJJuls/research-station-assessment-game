...
## 6. Unit 6 — Professional adult presentation

Files: `src/scenes/UtilityCoreDeckScene.ts`, `e2e/pilot_visual_capture.spec.ts`,
`docs/verification/screenshots-professional-pilot/*.png` (30 frames
regenerated by the capture spec, two renamed), this report.

### 6.1 Closed

- **U8-9 / ledger V28 (deck systems-board readout occluded).** The
  readout now sits on its own dark chip below the board decor
  (`board.y + 54`, depth 20, above every world sprite — decor depth is
  capped below 1 by `worldDepth`). The visual review confirms it is fully
  legible and unoccluded in every deck frame (`27`, `30`; behind the modal
  in `28`/`29` as expected). The Unit 7 ledger's "fixed" claim for V28 is
  now true; the row was not edited (the evidence, not the ledger, is what a
  reader cites).
- **Manifold line (new finding F3 of the visual review).** The
  `CORE FEED MANIFOLD` readout carried the three feed states with no
  backing chip and ran under a light steel panel, so the third feed's
  glyph could not be resolved at 800×600. It now has the same dark chip as
  its siblings. (Applied after the full sweep released the dev server;
  deck leg re-captured and the deck/closure specs re-run afterwards.)
- **U8-10 (frames that did not show what their names claimed).**
  `15-decoder-bank.png` → `15-laboratory-phase-benches.png` (the capture
  spec deliberately shoots the phase benches in world view) and
  `30-shift-complete.png` → `30-record-closed-feeds-pending.png` (the deck
  immediately after record closure, feeds still down; the completed shift
  is captured by `pilot_closure_capture.spec.ts`). Names now state the
  content; the capture spec writes the new names.

### 6.2 Visual review (read-only) — other findings, disposition

| #           | Finding                                                                     | Sev   | Disposition                                                                                                                           |
| ----------- | --------------------------------------------------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------- |
| F2          | dialogue box stacks 12 px under the readout chip in frame 30                | MINOR | recorded; chip is single-line by construction                                                                                         |
| F4          | `DOOR · SEALED` chip crowds the door leaf                                   | MINOR | recorded (U7 backlog)                                                                                                                 |
| F5          | deck decoration outweighs the Shift Review Panel as a target                | MAJOR | recorded for the owner's visual backlog (hierarchy is an art decision; no mechanic)                                                   |
| F6          | route banner still says "Concourse east door" after arrival; west-wall sign | MINOR | recorded (copy is route-objective text; owner OD)                                                                                     |
| F7          | banner and dialogue repeat the feeds instruction                            | MINOR | recorded                                                                                                                              |
| F8          | label crowding top of deck frame                                            | MINOR | recorded                                                                                                                              |
| F10         | no Core Chamber interior frame in this set                                  | MAJOR | by design of this spec (finale captured by `pilot_closure_capture`); frames 28/29 keep their names as they show the deck review modal |
| F13         | hotbar caption `Fuse` clipped under the M02 overlay                         | MINOR | recorded (overlay hygiene V1/V12 did not apply to this HUD instance)                                                                  |
| F14         | empty inventory grids show no empty-state line                              | MAJOR | recorded for the owner (participant-facing copy; entry-state question)                                                                |
| F15–F20     | duplication, hierarchy and register inconsistencies in the overlays         | MINOR | recorded                                                                                                                              |
| F17         | two icon registers in the M02 tray (`R-07`/`R-09` diagonal stroke)          | MINOR | routed to the scientific reviewer/owner (could bias filing behaviour)                                                                 |
| F18         | decoder footer keybinding line dense and low-contrast                       | MAJOR | recorded (`WorkSurfaceScene`/decoder copy outside the U6 allowlist)                                                                   |
| F21, F22    | adult tone; no score/trait/validity/item-id leakage in any reviewed frame   | INFO  | confirmed                                                                                                                             |
| F23         | end-of-shift coverage tally at the irreversible decision                    | INFO  | already an owner decision (OD-3 / U8-11); unchanged                                                                                   |
| gameplay #9 | accent-button contrast ≈2.7:1 (`WorkSurfaceScene`)                          | MINOR | recorded (outside every V3 allowlist)                                                                                                 |

### 6.3 Verification (`--retries=0 --workers=1`, `PW_DEV_PORT=5352`)

`npm.cmd run lint:tsc` pass; scoped ESLint pass; `npm.cmd run build` pass;
`pilot_visual_capture` 3/3 legs (6.0 min) for the full set, then leg 3
again (3.3 min) after the manifold chip; `pilot_deck` and `pilot_closure`
