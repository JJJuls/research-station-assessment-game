...
| `engineer_report_accuracy_scored`      | —                                      | exact (FABLE-NEXT-04)   | One per submission, at report-CONTENT selection (the new stage between the verbatim response-mode options and the duty offer). **Unmapped raw telemetry** — deliberately NOT registered in CanonicalEventContext (engineer_hub_entered precedent); adding a Q09 registration is an explicit research-owner event-schema decision (see SCIENTIFIC-AUTHORITY-AND-OPEN-DECISIONS.md). **Payload placement (additive clarification, `control_error_count` precedent)**: `success` = boolean, true iff every checkable fact in the submitted claim matches the logged SessionState; `metadata.accuracy` = 0-1 proportion of checkable facts correct; supporting keys `metadata.report_mode` (`unprepared`/`prepared`/`supervised`, the existing submitted-event vocabulary), `metadata.facts_total`, `metadata.facts_correct`, `metadata.claimed_systems_repair_complete`, `metadata.claimed_field_kit_packed`, `metadata.actual_systems_repair_complete`, `metadata.actual_field_kit_packed`. The two checkable facts are completed_rooms ∋ systems_repair_room and prepared_items ∋ field_kit. Evaluation is silent (neutral acknowledgement, no grade shown). Raw input for the approved-but-missing `report_accuracy_score` derived variable — computing it is D2-family scoring work. |
...
| `engineer_supervision_completed`       | —                                      | exact (Final Core beat) | Emitted by FinalCoreScene on the resolve path while the relay duty is active (Q10, responsibility). Table refreshed in FABLE-NEXT-04's Q10 verification pass — the emission itself landed with the Final Core beat.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `engineer_supervision_skipped`         | —                                      | missing                 | Still unemitted — no distinct formal skip act exists (FinalCoreScene header note); declining at offer time is `engineer_supervision_declined`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `accepted_duty_unresolved`             | —                                      | exact (Final Core beat) | Emitted by FinalCoreScene at completion time while the accepted duty is still open (Q10, responsibility). Table refreshed in FABLE-NEXT-04's Q10 verification pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
6 exact-status rows on the report-submission path (5 legacy-aligned matches
plus `engineer_report_accuracy_scored` since FABLE-NEXT-04); the accepted-duty /
supervision follow-through mechanic (Q10's core task) is implemented across
EngineerScene (offer/accept/decline) and FinalCoreScene (complete/unresolved),
and spec-covered end-to-end (`engineer_hub_logging.spec.ts`,
`final_core_summary.spec.ts`).

FABLE-NEXT-04 notes:

- Report flow is now mode → content → duty offer. The response-mode options,
  their labels, feedback strings and event sequences (including the
  legacy-only `engineer_report_submitted_supervised` and
  `engineer_responsibility_*` names, and the open D5 mapping question) are
  preserved verbatim; the duty offer still logs
  `engineer_supervision_assigned` at the moment it is shown.
- The report-content stage presents the four combinations of the two
  checkable facts as operational status statements in fixed template order
  (exactly one fully accurate in any state). Mode-specific stage body:
  quick = from memory; evidence review = station log extract with the actual
  values; clarification = Kai names the two facts he needs (no answers).
- The evaluation is a silent substrate: no grade, no moral labels, identical
  neutral acknowledgement for every claim. Q09's outcome side remains open —
  the emitted event is unmapped until the research owner rules on a Q09
  registration.
...
| `inventory_checklist_opened`         | `inventory_prep_opened`           | exact (Wave 1A)       | **Emission placement**: fires on checklist ACTIONS only, never on prompt open (the mechanical rename would credit Q01 checklist use to shortcut players) and never on per-item mode engagement. Two sources: the legacy "Open the checklist..." option (beside legacy `inventory_checklist_used`) and the per-item console option "Check the kit requisition list." (FABLE-NEXT-02 — canonical name only; no legacy alias fires there).                                                                                                                                                                 |
| `inventory_item_sorted_correct`      | —                                 | exact (FABLE-NEXT-02) | Per-item mode: placing an item into the storage bin its tag names. One placement act = one event; `object_id` = registry `item_id`, `attempt_number` = that item's placement count. Placement feedback is neutral — errors surface only at the bench review.                                                                                                                                                                                                                                                                                                                                            |
| `inventory_item_misplaced`           | —                                 | exact (FABLE-NEXT-02) | Per-item mode: placing an item into a bin its tag does not name. Same `object_id`/`attempt_number` payload rule. A corrected re-placement is derivable from the event sequence (a correct-placement event for the same `object_id` with a higher `attempt_number`) — see the FABLE-NEXT-02 notes below.                                                                                                                                                                                                                                                                                                 |
| `inventory_sequence_followed`        | `inventory_systematic_prep`       | exact (Wave 1A)       | Two mutually exclusive sources: additive beside the unchanged legacy name on the legacy systematic option; in per-item mode judged exactly once, at the first moment the kit holds all required items, firing iff their first kit placements happened in checklist order.                                                                                                                                                                                                                                                                                                                               |
| `inventory_sequence_completed`       | —                                 | exact (FABLE-NEXT-02) | Emitted once per session (console/task context) when every registry item has been placed at some destination. **Emitted but unregistered and unmapped**: no `CANONICAL_EVENT_CONTEXT` entry and no matrix Events-column listing, so it logs as raw telemetry with no `study_item_ids`/`construct_id`. Registering/mapping it is an open research-owner event-schema decision — not promoted here.                                                                                                                                                                                                       |
| `inventory_verification_skipped`     | —                                 | exact (Wave 1A)       | Legacy shortcut path (skips by meaning) + the plausible skip option of BOTH verification stages (legacy option-2 chain and per-item close-out).                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `inventory_verified_complete`        | `inventory_kit_verified`          | exact (Wave 1A)       | Additive alongside legacy on sort-and-verify; sole event on the run option of BOTH verification stages (legacy option-2 chain and per-item close-out). In per-item mode it records the verification ACT and fires regardless of kit completeness — the result is shown in prompt text and derivable from the stream.                                                                                                                                                                                                                                                                                    |
| `correct_tool_selected`              | `inventory_required_tools_packed` | exact (Wave 1A)       | Additive alongside legacy on the legacy checklist option; in per-item mode, packing a kit-required item into the kit crate (`object_id` = registry `item_id`, `attempt_number` payload).                                                                                                                                                                                                                                                                                                                                                                                                                |
| `wrong_tool_selected`                | —                                 | exact (FABLE-NEXT-02) | Per-item mode: packing a non-kit (stray) item into the kit crate. Same `object_id`/`attempt_number` payload rule.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `prepared_tool_used`                 | —                                 | missing               | Registered in `CANONICAL_EVENT_CONTEXT` (Q03 — registration ≠ emission) but **unemitted**: the Q03 prepared-tool retrieval episode is a later cross-room unit, so emission stays a candidate for that pass.                                                                                                                                                                                                                                                                                                                                                                                             |
| `readiness_verified`                 | `inventory_kit_verified`          | missing               | Still deliberately **unemitted** after FABLE-NEXT-02: not matrix-listed, unregistered, and no distinct readiness action exists in either verification stage — never double-logged with `inventory_verified_complete` from one click.                                                                                                                                                                                                                                                                                                                                                                    |
| `missing_item`                       | `inventory_required_item_missed`  | exact (Wave 1A)       | Unmapped (not matrix-listed; no registration). Two sources: additive beside legacy on the legacy shortcut path (console object context); per-item close-out logs it once per session per still-missing requisition item with `object_id` = registry `item_id`, at the FIRST close-out review that surfaces it — i.e. pre-correction. **D2 scoring rule**: a later D2-family pass must evaluate omissions against final `prepared_items`/`field_kit` state, never by treating a `missing_item` event as an unresolved omission — the bench-review correction may have resolved it after the event fired. |
| `workspace_tidy_confirmed`           | `inventory_workspace_sorted`      | exact (Wave 1A)       | Additive alongside legacy on sort-and-verify; also the sort/reset option of BOTH cleanup stages (legacy chain and per-item).                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `workspace_left_disordered`          | `inventory_disorganized_action`   | exact (Wave 1A)       | Additive alongside legacy on the shortcut path; also the leave option of BOTH cleanup stages.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `cleanup_completed`                  | `inventory_cleanup_completed`     | exact (Wave 1A)       | Additive alongside legacy (prefix differs only); paired with `workspace_tidy_confirmed` on the sort/reset option of BOTH cleanup stages.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `final_core_missing_item_flagged`    | —                                 | exact (Wave 1A)       | Emitted at Final Core room entry (see the Final Core table). Inventory feeds the SessionState source on every path: legacy and per-item completion both leave `field_kit` out of `prepared_items` when the kit is incomplete.                                                                                                                                                                                                                                                                                                                                                                           |
| `final_core_workspace_issue_flagged` | —                                 | exact (Wave 1A)       | Emitted at Final Core room entry (see the Final Core table); `workspace_status = disordered` is written only by the explicit leave choice on a cleanup stage (legacy or per-item) or the legacy shortcut option.                                                                                                                                                                                                                                                                                                                                                                                        |
| —                                    | `inventory_prep_shortcut`         | legacy-only           | Kept verbatim on the shortcut path (audit-first port).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

**FABLE-NEXT-02 per-item preparation notes (2026-07-18 refresh; docs-only —
no gameplay source, registration, mapping or scoring change):**

- The per-item mode is an additive fourth console option ("Stage the kit
  yourself at the bench, item by item."); the three legacy options and their
  event cascades are preserved verbatim, so every canonical alias above keeps
  its legacy emission source too. Selecting the mode emits NO event — no
  approved name exists for mode engagement, and its occurrence is derivable
  from the per-item events that follow.
- **Cleanup is binary restore-vs-leave** on every close-out path (including
  the rushed one, so disorder is always chosen, never asserted). Contract
  §R4's "restore/partial/leave" partial-restoration option is **not
  implemented**: no approved event name exists for a partial restore (the
  measurement specification lists `partial_cleanup_count` only as a candidate
  derived indicator). This stays an OPEN research-owner decision — recorded,
  not silently narrowed.
- **Unemitted candidates** (never promoted without an explicit event-schema
  decision from the research owner): `inventory_item_corrected` (correction
  telemetry stays derivable — a correct placement event following
  `inventory_item_misplaced`/`wrong_tool_selected` for the same `object_id`
  with a higher `attempt_number`); `cleanup_opportunity_shown` (the cleanup
  stage is structurally guaranteed on every close-out path, so the
  opportunity is derivable from event order); `prepared_tool_used` (later
  cross-room unit — see its row).

After FABLE-NEXT-02, every canonical Inventory event is emitted except
`prepared_tool_used` (later cross-room unit) and `readiness_verified`
(deliberately unemitted — no distinct action), with
`inventory_sequence_completed` emitted but unregistered/unmapped as noted
above. The pre-FABLE-NEXT-02 "zero exact string matches" assessment is
superseded; the underlying shortcut-vs-systematic-vs-cleanup mechanic
(Q01-Q04, Q30) now also has real per-item placement substrate.

### Hazard Control Room (`hazard_control_room`)

| Canonical (V3)             | Current prototype          | Status              | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------------- | -------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hazard_room_entered`      | —                          | exact (Hazard beat) | Emitted by HazardScene on every entry (repair/inventory precedent; unmapped — no Events-column listing).                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `hazard_issue_created`     | —                          | missing             | No documented emission semantics or resolve mechanic (task-design decision, not an audit-first port). `SessionState.hazard_status` now records the decision outcome for a future pass.                                                                                                                                                                                                                                                                                                                                                                           |
| `hazard_issue_resolved`    | —                          | missing             |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `final_hazard_issue`       | —                          | missing             | Cross-room propagation to Final Core not implemented.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `hazard_route_avoided`     | `hazard_avoidance`         | approved            | **User ruling D1, 2026-07-12 (Option A)**: new additive canonical event `hazard_route_avoided` for the third choice ("avoid uncertain route"). `study_item_ids: []`, `construct_id` unset — raw behavioural telemetry only, never construct-scored, never added to ScoringManager formulas, no questionnaire-item mapping inferred. No success semantics documented for this branch, so none assigned. **Now emitted** additively alongside the legacy event on the avoid option — status: exact (Hazard beat).                                                  |
| —                          | `hazard_avoidance`         | legacy-only         | Preserved verbatim per user ruling D1 (existing semantics unchanged; the legacy `abandonment_count` derivation continues to read this event). Superseded as a canonical question by the approved `hazard_route_avoided` row above. `hazard_info_checked` remains reserved exclusively for actual information-checking behaviour — it is NOT the canonical representation of route avoidance. Route avoidance stays analytically distinguishable from info-checking, informed continuation, reckless continuation, and task abandonment (each has its own event). |

Strongest alignment of any room (4 exact matches on the core decision path).
Ruling record: `docs/expansion/reviews/WAVE1-USER-DECISION-BRIEF.md` §A (D1).
