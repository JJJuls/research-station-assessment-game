...
- **Last commit**: `f1db32f` — docs(expansion): Wave 1 plan
- **Completed**:
...
- **Exact next action**: commit this state file, then implement **U1 —
  station registry and routing** (`src/world/stationRegistry.ts`; HubScene
  door ring + SceneRouter aliases read from it; zero behaviour change;
  build + tsc; commit).
- **Then**: U2 task-state factory → U3 N-option/multi-stage prompts →
  U4 canonical event-context registrations → U5 mission-state helpers →
  U6 e2e fixtures → station beats in plan order.
